      ...
      <aside class="side-panel">
        ...
        <section class="panel-block invite-section">
          ...
          <div class="invite-form">
            ...
            />
            <button class="invite-btn" @click="sendInvitation" :disabled="!inviteEmail.trim()">
              发送邀请
            </button>
          </div>
        </section>
...
// import projectService from '@/api/projectService' // 根据实际路径调整
...
interface Member {
  ...
  name: string
  email: string
  role: string // 'owner' | 'admin' | 'editor' | 'executor' | 'viewer'
}
...
const loadData = async () => {
  ...
  try {
    ...
    //   projectService.getJoinRequests?.(props.projectId) ?? []
    // ])
    // members.value = membersData
    // joinRequests.value = requestsData
  } catch (error) {
...
const sendInvitation = async () => {
  ...
  if (!email) return
  try {
    // await projectService.inviteMember(props.projectId, { email })
    inviteEmail.value = ''
  } catch (err) {
...
const updateRole = async (memberId: string, newRole: string) => {
  try {
    // await projectService.updateMemberRole(props.projectId, memberId, { role: newRole })
...
const handleRequest = async (requestId: string, action: 'approve' | 'reject') => {
  try {
    if (action === 'approve') {
      // await projectService.approveJoinRequest?.(props.projectId, requestId)
    } else {
      // await projectService.rejectJoinRequest?.(props.projectId, requestId)
    }
