<template>
  <div class="order-manager">
    <header class="module-header">
      <div class="header-content">
        <h2 class="page-title">订单治理中枢</h2>
        <p class="md-subtitle">收款确认 · 状态追踪 · 异常处理</p>
      </div>
    </header>

    <!-- 统计条 -->
    <div class="stats-bar">
      <span class="stat"><i class="sdot pending"></i> 待确认 <b>{{ stat.pending }}</b></span>
      <span class="stat"><i class="sdot confirmed"></i> 已收款 <b>{{ stat.confirmed }}</b></span>
      <span class="stat"><i class="sdot processing"></i> 服务中 <b>{{ stat.processing }}</b></span>
      <span class="stat"><i class="sdot completed"></i> 已完成 <b>{{ stat.completed }}</b></span>
      <span class="stat"><i class="sdot cancelled"></i> 已取消 <b>{{ stat.cancelled }}</b></span>
    </div>

    <!-- 工具栏 -->
    <div class="panel-toolbar">
      <div class="filter-row">
        <input
          v-model="search"
          @keyup.enter="loadOrders"
          class="ink-input"
          placeholder="搜索订单号 / 老板 / 打手 GUID (回车)..."
        />
        <select v-model="statusFilter" class="ink-select" @change="loadOrders">
          <option value="">全部状态</option>
          <option value="pending">待处理</option>
          <option value="accepted">已接单</option>
          <option value="processing">服务中</option>
          <option value="completed">已完成</option>
          <option value="cancelled">已取消</option>
        </select>
      </div>
      <button class="btn-refresh" @click="loadOrders" :disabled="loading">
        {{ loading ? '同步中...' : '刷新' }}
      </button>
    </div>

    <!-- 表格 -->
    <div class="table-card">
      <table class="ink-table">
        <thead>
          <tr>
            <th width="180">订单号</th>
            <th width="140">游戏 / 类型</th>
            <th width="140">老板 → 打手</th>
            <th width="90">金额</th>
            <th width="100">订单状态</th>
            <th width="100">收款状态</th>
            <th width="140">创建时间</th>
            <th width="180" class="text-right">操作</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="o in orders" :key="o.orderNo">
            <td class="mono">{{ o.orderNo }}</td>
            <td>
              <div class="ot-name">{{ o.gameName }}</div>
              <div class="ot-desc">{{ o.orderTypeName }}</div>
            </td>
            <td>
              <div class="mono font-sm">{{ shortId(o.customerId) }}</div>
              <div class="mono font-sm text-muted">→ {{ o.operatorName || shortId(o.operatorUserId) }}</div>
            </td>
            <td class="mono price">¥{{ o.price }}</td>
            <td>
              <span :class="['status-pill', o.status]">
                {{ statusLabel(o.status) }}
              </span>
            </td>
            <td>
              <span v-if="o.paidConfirmedAt" class="paid-badge">✓ 已收款</span>
              <span v-else class="unpaid-badge">未确认</span>
            </td>
            <td class="mono font-sm">{{ fmtTime(o.createdAt) }}</td>
            <td class="text-right actions">
              <button
                v-if="!o.paidConfirmedAt && o.status === 'pending'"
                class="btn-text"
                @click="confirmPaid(o)"
              >确认收款</button>
              <button
                v-if="o.status === 'pending'"
                class="btn-text danger"
                @click="cancelOrder(o)"
              >取消</button>
              <span v-if="o.paidConfirmedAt || o.status !== 'pending'" class="text-muted">—</span>
            </td>
          </tr>
          <tr v-if="orders.length === 0 && !loading">
            <td colspan="8" class="empty-cell">暂无订单</td>
          </tr>
        </tbody>
      </table>
    </div>

    <!-- 分页 -->
    <div v-if="total > 0" class="pagination">
      <span class="page-info">共 {{ total }} 条</span>
      <button class="btn-page" :disabled="page === 1" @click="changePage(page - 1)">上一页</button>
      <span class="page-cur">{{ page }} / {{ totalPages }}</span>
      <button class="btn-page" :disabled="page >= totalPages" @click="changePage(page + 1)">下一页</button>
    </div>
  </div>
</template>

<script setup lang="ts">
import { ref, reactive, computed, onMounted } from 'vue';
import request from '@/utils/request';

interface OrderItem {
  orderNo: string;
  status: string;
  price: number;
  remark?: string;
  createdAt: string;
  paidConfirmedAt?: string | null;
  gameCode: string;
  gameName: string;
  orderTypeCode: string;
  orderTypeName: string;
  customerId: string;
  customerName: string;
  operatorUserId: string;
  operatorName: string;
  params?: Record<string, string>;
}

const orders = ref<OrderItem[]>([]);
const loading = ref(false);
const search = ref('');
const statusFilter = ref('');
const page = ref(1);
const pageSize = 20;
const total = ref(0);

const totalPages = computed(() => Math.max(1, Math.ceil(total.value / pageSize)));

const stat = reactive({
  pending: 0,
  confirmed: 0,
  processing: 0,
  completed: 0,
  cancelled: 0
});

const statusLabel = (s: string) => ({
  pending: '待处理',
  accepted: '已接单',
  processing: '服务中',
  completed: '已完成',
  cancelled: '已取消'
}[s] || s);

const shortId = (uid: string) => uid ? uid.substring(0, 8).toUpperCase() : '—';

const fmtTime = (iso: string) => {
  if (!iso) return '—';
  const d = new Date(iso);
  const pad = (n: number) => String(n).padStart(2, '0');
  return `${pad(d.getMonth() + 1)}-${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}`;
};

const loadOrders = async () => {
  loading.value = true;
  try {
    const res: any = await request.get('/admin/club/orders', {
      params: {
        search: search.value.trim() || undefined,
        status: statusFilter.value || undefined,
        page: page.value,
        pageSize
      }
    });
    const payload = res?.data ?? res;
    orders.value = payload?.items ?? [];
    total.value = payload?.total ?? 0;
    recalcStat();
  } catch (e) {
    console.error('拉取订单失败', e);
    orders.value = [];
    total.value = 0;
  } finally {
    loading.value = false;
  }
};

const recalcStat = () => {
  const list = orders.value;
  stat.pending = list.filter(o => o.status === 'pending' && !o.paidConfirmedAt).length;
  stat.confirmed = list.filter(o => o.paidConfirmedAt).length;
  stat.processing = list.filter(o => o.status === 'processing' || o.status === 'accepted').length;
  stat.completed = list.filter(o => o.status === 'completed').length;
  stat.cancelled = list.filter(o => o.status === 'cancelled').length;
};

const confirmPaid = async (o: OrderItem) => {
  if (!confirm(`确认订单【${o.orderNo}】已收到 ¥${o.price} 款项？`)) return;
  try {
    await request.post(`/admin/club/order/${o.orderNo}/confirm-paid`);
    await loadOrders();
  } catch (e: any) {
    alert(e?.response?.data?.message || e.message || '操作失败');
  }
};

const cancelOrder = async (o: OrderItem) => {
  const reason = prompt(`取消订单【${o.orderNo}】？\n请输入取消原因（可留空）:`);
  if (reason === null) return;
  try {
    await request.post(`/admin/club/order/${o.orderNo}/cancel`, { reason });
    await loadOrders();
  } catch (e: any) {
    alert(e?.response?.data?.message || e.message || '操作失败');
  }
};

const changePage = (p: number) => {
  if (p < 1 || p > totalPages.value) return;
  page.value = p;
  loadOrders();
};

onMounted(loadOrders);
</script>

<style scoped>
.order-manager { display: flex; flex-direction: column; gap: 20px; animation: fadeIn 0.3s ease; }
.module-header { margin-bottom: 8px; }
.page-title { font-size: 1.6rem; font-weight: 700; color: #111; margin: 0; }
.md-subtitle { font-size: 0.85rem; color: #888; margin: 6px 0 0; }

.stats-bar {
  display: flex; flex-wrap: wrap; gap: 24px;
  padding: 12px 20px; background: #fafafa; border-radius: 6px;
  font-size: 0.85rem;
}
.stat { display: flex; align-items: center; gap: 8px; color: #666; }
.stat b { color: #111; font-weight: 700; }
.sdot { width: 8px; height: 8px; border-radius: 50%; }
.sdot.pending    { background: #f97316; }
.sdot.confirmed  { background: #2563eb; }
.sdot.processing { background: #a855f7; }
.sdot.completed  { background: #16a34a; }
.sdot.cancelled  { background: #999; }

.panel-toolbar {
  display: flex; justify-content: space-between; align-items: center;
  gap: 16px;
}
.filter-row { display: flex; gap: 12px; flex: 1; max-width: 820px; }
.filter-row .ink-input { flex: 2; }

.ink-input, .ink-select {
  border: 1px solid #e0e0e0; padding: 10px 14px; border-radius: 4px;
  font-size: 0.85rem; outline: none; background: #fff; font-family: inherit;
}
.ink-input:focus, .ink-select:focus { border-color: #1a1a1a; }
.ink-select { cursor: pointer; min-width: 140px; }

.btn-refresh {
  background: #fff; border: 1px solid #e0e0e0; padding: 10px 20px;
  border-radius: 4px; color: #555; cursor: pointer; font-size: 0.85rem;
}
.btn-refresh:hover { border-color: #1a1a1a; color: #000; }
.btn-refresh:disabled { opacity: 0.5; cursor: not-allowed; }

.table-card {
  background: #fff; border: 1px solid #f0f0f0; border-radius: 8px;
  overflow: hidden;
}
.ink-table { width: 100%; border-collapse: collapse; font-size: 0.88rem; }
.ink-table th {
  padding: 14px 16px; background: #fcfcfc; color: #888; text-align: left;
  font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.5px;
  border-bottom: 2px solid #111;
}
.ink-table td { padding: 14px 16px; border-bottom: 1px solid #f7f7f7; vertical-align: middle; }
.ink-table tr:hover td { background: #fafafa; }
.text-right { text-align: right; }
.text-muted { color: #999; }
.mono { font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace; }
.font-sm { font-size: 0.78rem; }
.empty-cell { text-align: center; padding: 60px !important; color: #bbb; font-style: italic; }

.ot-name { font-weight: 600; color: #111; font-size: 0.85rem; }
.ot-desc { margin-top: 2px; font-size: 0.72rem; color: #999; }
.price { color: #c2410c; font-weight: 600; }

.status-pill {
  display: inline-block; padding: 3px 10px;
  font-size: 0.72rem; font-weight: 600; border-radius: 3px;
}
.status-pill.pending    { background: #fff7ed; color: #c2410c; }
.status-pill.accepted   { background: #eff6ff; color: #2563eb; }
.status-pill.processing { background: #f3e8ff; color: #a855f7; }
.status-pill.completed  { background: #e3fcef; color: #00875a; }
.status-pill.cancelled  { background: #f5f5f5; color: #999; }

.paid-badge {
  display: inline-block; padding: 3px 8px;
  font-size: 0.7rem; font-weight: 600;
  color: #00875a; background: #e3fcef; border-radius: 3px;
}
.unpaid-badge {
  display: inline-block; padding: 3px 8px;
  font-size: 0.7rem; font-weight: 600;
  color: #c2410c; background: #fff7ed; border-radius: 3px;
}

.btn-text {
  background: none; border: none; color: #2563eb; cursor: pointer;
  font-size: 0.8rem; font-weight: 700; margin-left: 12px; padding: 0;
}
.btn-text:hover { text-decoration: underline; }
.btn-text.danger { color: #dc2626; }

.pagination {
  display: flex; justify-content: flex-end; align-items: center;
  gap: 12px; font-size: 0.85rem; color: #666;
}
.page-info { margin-right: auto; }
.page-cur { font-family: monospace; }
.btn-page {
  background: #fff; border: 1px solid #ddd; padding: 6px 14px;
  border-radius: 4px; cursor: pointer; font-size: 0.8rem;
}
.btn-page:not(:disabled):hover { border-color: #111; }
.btn-page:disabled { opacity: 0.4; cursor: not-allowed; }

@keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
</style>